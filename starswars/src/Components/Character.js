...
import Content from '../Components/Charactercontent'
            ...
            <div class="col-md-9">
                ...
                <div className="ui search" style={{marginLeft: 300}}>
                    <div className="ui icon input search">
                        <input className="prompt" type="text" placeholder="Search Character" ref="search" onChange={() => { this.cari(this.refs.search.value); }} />
                ...
                {this.state.get.filter(item =>
                    (new RegExp(this.state.search, 'gi')).test(item.name))
                    .map(item =>
                        <Content {...item} />
                    )}
            </div>
